...
import sklearn
from .models import odis,ipl,t20men
from .serializers import odisSerializer,t20Serializer,iplSerializer
...
@api_view(['GET'])
def get_odis_detail(request):
    Data=odis.objects.all()
    serializers=odisSerializer(Data,many=True)
    return Response({"Data":serializers.data})


@api_view(['GET'])
def get_ipl_detail(request):
    Data=ipl.objects.all()
    serializers=iplSerializer(Data,many=True)
    return Response({"Data":serializers.data})

@api_view(['GET'])
def get_t20_detail(request):
    Data=t20men.objects.all()
    serializers=t20Serializer(Data,many=True)
    return Response({"Data":serializers.data})


@api_view(['POST'])
def save_t20_detail(request):
    data=request.data
    t20=t20men.objects.create(**data)
    #serializer=t20Serializer(t20,many=False)
    return Response({"message":"saved Succesfully"})


@api_view(['POST'])
def save_odis_detail(request):
    data=request.data
    odi=odis.objects.create(**data)
    serializer=odisSerializer(odi,many=False)
    return Response({"message":"saved Succesfully"})

@api_view(['POST'])
def save_ipl_detail(request):
    data=request.data
    IPL=ipl.objects.create(**data)
    serializer=iplSerializer(IPL,many=False)
    return Response({"message":"saved Succesfully"})
